...
**Полное описание:** см. [docs/DESIGN_SYSTEM.md → Tailwind Plus Elements](docs/DESIGN_SYSTEM.md#tailwind-plus-elements--где-используем)

Краткие правила:
- Подключать **только если** на странице есть компоненты `el-*`
- Подключать **перед** `nav.js` как `type="module"`
- **Не редактировать** вендорный файл
...
**Decision flow:**
1. Есть `el-*` компонент → используем его
2. Нет в Plus, но логика на 2+ страницах → добавляем в `nav.js`
3. Уникально для одной страницы → короткий inline с guard-проверками
...
- [ ] Нет inline-полифиллов для `el-*` компонентов (ensureTabsFallback и подобные) — функционал уже есть в `tailwindplus-elements.js`
...
**Эталон:** `src/html/portret-na-zakaz/style/portret-maslom.html` — правильная реализация с `el-dialog`.

**На всех остальных страницах:** OAuth модальные окна **УДАЛЯЮТСЯ** — программист Bitrix добавит их на этапе интеграции.

**Структура el-dialog (обязательна!):**
```html
<el-dialog>
    <dialog id="review-modal" class="m-auto p-0 border-none bg-transparent backdrop:bg-black/60">
        <el-dialog-panel>
            <div class="bg-dark text-white rounded-lg shadow-2xl max-w-md w-full p-6">
                ...
                <button command="close" commandfor="review-modal">Закрыть</button>
            </div>
        </el-dialog-panel>
    </dialog>
</el-dialog>
...
**Важно:** Стили (`bg-dark`, `rounded-lg` и т.д.) на внутреннем `<div>`, НЕ на `<el-dialog-panel>`.

**Кнопка открытия:**
```html
<button command="show-modal" commandfor="review-modal">Оставить отзыв</button>
```

**Провайдеры (5 шт):** Яндекс, ВКонтакте, Google, Mail.Ru, Одноклассники

**Правила:**
- `<el-dialog>` оборачивает `<dialog>`, внутри `<el-dialog-panel>` → `<div>` со стилями
