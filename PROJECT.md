...
- ⏳ Проверка DPI загруженного фото (не реализована — uploader хранит `width`/`height`, но индикатор в UI отсутствует)
...
- ⏳ DPI-индикатор (предупреждение о качестве) — uploader хранит `width`/`height`, но UI-индикатор не реализован
