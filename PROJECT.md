...
- ⏳ **Стабилизация и развитие калькуляторов** → см. [План CALC: стабилизация и развитие](#план-calc-стабилизация-и-развитие-17-февраля-2026)
...
- ✅ Создан универсальный JS-компонент `CalcInit()` (~850 строк → вырос до ~1 500 строк) с публичным API и конфигурацией через объект
...
- ~~Проверка DPI загруженного фото~~ — отменено
...
|---|---|---|
| **Движок** | Калькулятор встроен как серверный компонент 1С-Битрикс, рендерится на стороне сервера | Полностью клиентский JS-калькулятор `calc.js` (~1 500 строк) |
...
В Tailwind-проекте — полностью клиентская замена. `CalcInit(cfg)` принимает конфиг и строит UI. Цены — из единого конфига `prices.js` (fallback: `DEFAULT_PRICES` в `calc.js`, строки 82–111). Формула расчёта — в функции `calculate()` (строка ~1235). Отправка заказа — заглушка (`console.log` + `alert`).
...
| `foto-na-kholste-sankt-peterburg.html` | **Работает** | `CalcInit({ type: 'canvas', prices: MUSE_PRICES.canvas })` |
| `pechat-na-kholste-sankt-peterburg.html` | **Заглушка** | Текст «Калькулятор будет добавлен при интеграции с Bitrix» |
| `portret-maslom.html` | **✅ Работает** | `CalcInit({ type: 'portrait', prices: MUSE_PRICES.portrait, tooltips })` |
| 22 страницы portret-na-zakaz | **Заглушка** | Идентичный dashed-box |

**Итого: 2 рабочих калькулятора, 1 демо-эталон, 23 заглушки.**
...
- **Оптимизация изображений на клиенте:** загруженные фото ресайзятся через `<canvas>` — превью ≤ 1000 px, миниатюры ≤ 120 px, JPEG quality 0.85. Вместо `FileReader.readAsDataURL` используется `URL.createObjectURL` → blob-URL отзывается при удалении/очистке (`revokeObjectURL`). Экономия памяти: ~4 МБ/фото → ~53 КБ/фото.

**НЕ работает / заглушка:**
- Отправка заказа = `console.log` + `alert`
- ~~DPI-индикатор~~ — отменено
- Серверная валидация цен: отсутствует
- ~~Портретные опции~~ — ✅ реализованы в Сессии 4–5: `buildPortraitSections()`, лица, гель, акрил, масло, поталь, цифровой макет
...
| Портретные секции | Генерируются JS (`buildPortraitSections()`) при `type: 'portrait'` | Не применимо (тип canvas) |
...
| 4 | ~~Портретные опции не реализованы~~ | ✅ Закрыт | — | Реализовано в Сессии 4–5: `buildPortraitSections()`, `toggleDigitalMockupMode()`, все опции интегрированы в `calculate()`. |
...
Архитектура (`shared` + `merge`):
```
prices.js
  │
  ├─ shared { print*, stretcher*, varnish, giftWrap*, frame* }
  │      └─ общие параметры — меняете в одном месте
  │
  ├─ canvas:   merge({ processingOptions })      ← обработка только для холста
  └─ portrait: merge({ face*, digital*, gel, acrylic, oil, potal })
       │
       └─ CalcInit({ prices: MUSE_PRICES.portrait })
          CalcInit({ prices: MUSE_PRICES.canvas })
              └─ calc.js: PRICES = cfg.prices || DEFAULT_PRICES
```

Захардкоженных цен в HTML нет — `<select>` обработки заполняется из `PRICES.processingOptions` при инициализации.
...
| Поталь | `S × 0.3` | 360 р. |
| Обработка: базовая | 0 р. | только canvas |
| Обработка: оптимальная | 300 р. | только canvas |
| Обработка: премиальная | 900 р. | только canvas |
...
- ✅ Цены — единый конфиг `prices.js` (`shared` + `merge`), все цены масла реализованы
- ✅ Очистка мёртвого кода — удалены `processingOptions`, `digitalMockupFixed`, `priceSize`, `sizeCost`, `gallerySurcharge`, `noFrameDiscount`, `showProcessing`/`showUpload` из HTML
- ✅ HTML `<select>` обработки — опции заполняются из `prices.js`, захардкоженные удалены
...
## План CALC: стабилизация и развитие (17 февраля 2026)

> Принятый порядок: **сначала довести калькулятор до ума, потом тиражировать.**
> Отправка формы заказа — задача интеграции с Битрикс (не входит в этот план).

### Фаза 1 — Синхронизация эталона (calc.html)

**Цель:** сделать `calc.html` реальным source of truth, вобрав лучшее из `portret-maslom.html`.

| # | Задача | Файл | Статус |
|---|--------|------|--------|
| 1.1 | Убрать `container` с `<section>`, добавить вложенный `<div class="container">` | `calc.html` | ✅ |
| 1.2 | Обновить паддинги на `py-16 lg:py-20` | `calc.html` | ✅ |
| 1.3 | Добавить `content-auto` на `<section>` | `calc.html` | ✅ |
| 1.4 | Добавить `id="varnish-gift-section"` на секцию лака/упаковки | `calc.html` | ✅ |
| 1.5 | Переместить `<dialog>` (frame-modal, lightbox) внутрь `<section id="calc">` | `calc.html` | ✅ |
| 1.6 | Стандартизировать `<script defer src="...">` (порядок атрибутов + отступы) | `calc.html` | ✅ |
| 1.7 | Синхронизировать `foto-na-kholste-sankt-peterburg.html` с обновлённым эталоном | `foto-na-kholste` | ✅ |
| 1.8 | Синхронизировать `portret-maslom.html` (убрать нежелательные расхождения) | `portret-maslom` | ✅ |

### Фаза 2 — Решение дублирования цен

**Цель:** одно место для цен, минимум риска рассинхронизации.

| # | Задача | Файл | Статус |
|---|--------|------|--------|
| 2.1 | Заменить `DEFAULT_PRICES` (22 значения) → `MINIMAL_FALLBACK` (только чтобы не упало) | `calc.js` | ✅ |
| 2.2 | Убрать inline-fallback `\|\| 32`, `\|\| 68`, `\|\| 1200` из формул | `calc.js` | ✅ |
| 2.3 | Сделать `merge(DEFAULT, cfg.prices)` вместо «всё или ничего» | `calc.js` | ✅ |
| 2.4 | Добавить комментарий-инструкцию в `prices.js`: «при смене цен → проверить SEO-тексты» | `prices.js` | ✅ |

### Фаза 3 — Оптимизация производительности

**Цель:** убрать лишнюю работу при каждом изменении параметров.

| # | Задача | Файл | Статус |
|---|--------|------|--------|
| 3.1 | Debounce на ввод размеров (150–200 мс) — сейчас `renderFrameCatalog()` на каждый keystroke | `calc.js` | ✅ |
| 3.2 | Кэширование DOM-элементов (`.wrap-btn`, `.frame-option`, `.frame-image-preview`) | `calc.js` | ✅ |
...
| # | Задача | Файл | Статус |
|---|--------|------|--------|
| 4.1 | ~~DPI-индикатор~~ — отменено (не нужен расчёт качества фото при загрузке) | — | ❌ отменено |
| 4.2 | Вынести тексты tooltips в общий конфиг (чтобы не копировать 8 текстов на 22 страницы) | `prices.js` | ✅ |
| 4.3 | ~~Удалить `draft/calculator.html`~~ — файл уже удалён | `draft/` | ✅ |
| 4.4 | Tooltips для canvas-страниц: «Покрытие лаком» и «Подарочная упаковка» — убраны описательные тексты из UI, добавлены кнопки `?` с тултипами. Тексты в `MUSE_TOOLTIPS.canvas`. JS: `enhanceStaticVarnishGift()` в `calc.js` | `prices.js`, `calc.js`, `calc.html`, `foto-na-kholste` | ✅ |
| 4.5 | Итоговая строка калькулятора: удалены «Итого», перечёркнутая цена, бейдж `-20%`. На ПК: «Цена со скидкой 20% — N р.» (текст слева, цена справа — в столбик). На мобильном sticky bar: цена + текст скидки + кнопка «Заказать». JS: удалена логика `fakeOldPrice` / `discountBadge` | `calc.html`, `foto-na-kholste`, `portret-maslom.html`, `calc.js` | ✅ |

### Фаза 5 — Тиражирование на портретные страницы
...
```
Фаза 1 (эталон) ──┬──→ Фаза 2 (цены) ──→ Фаза 3 (скорость) ──→ Фаза 5 (тиражирование)
                   └──→ Фаза 4 (доработки) ─────────────────────↗
```