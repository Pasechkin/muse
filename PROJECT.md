...
- Масштабируем V1 на все 23+ страницы по плану rollout (Batch 0–4).
- Конфиг страницы — через `<script type="application/json" id="calc-config">`.
- Серверная интеграция (Hybrid/API) — только при появлении реальной потребности (частые изменения цен/багетов, запрос от бизнеса).

J. Pilot Gate (2 страницы → цены → скорость)
Пилот 1: foto-na-kholste-sankt-peterburg.html.
Пилот 2: portret-maslom.html.
Цены: вынести в единый конфиг (один источник для обеих страниц) + fallback.
Проверка скорости в реальности: Lighthouse + Web Vitals (LCP, INP, CLS) до/после.
Decision Gate: если 2 страницы стабильны и быстры — масштабируем; если нет — включаем Hybrid для управления данными (сначала цены, затем багет).

Used 1 reference
Planning research and verification steps
Проверю выводы из диалога на фактах: сверю PROJECT.md, код калькулятора и признаки live-реализации, затем дам список что подтверждено, что спорно и что стоит поправить. Сначала соберу только проверяемые данные из локальных файлов и live-страниц.

Searched for multiple regular expressions and planned fetching
Собрал первичную сверку: формулы и статусы страниц подтверждаются, но уже вижу конфликт в PROJECT.md — одновременно зафиксированы и «рекомендация V2 Hybrid», и «принято V1». Сейчас проверю live-страницы на признаки внешнего калькулятора и дам финальный вердикт по спорным пунктам.

Fetched https://muse.ooo/pechat-na-kholste-sankt-peterburg/foto-na-kholste-sankt-peterburg/
Fetched https://muse.ooo/portret-na-zakaz/portret-maslom/
Fetched https://framemuse.ru/
Verifying redirect claim uncertainty
Проверка live подтвердила контент секций «Цена», но из доступных инструментов нельзя надёжно подтвердить HTTP-редирект и источники скриптов по тегам. Сейчас доберу точные факты по зависимостям проекта и уже дам итог: что подтверждаю, что опровергаю и что уточняю.

Reviewed package.json and searched for regular expressions
Да — перепроверил по коду и live-данным. Ниже дельта-вердикт.

Подтверждаю

В проекте реально рабочий runtime-калькулятор: формулы и цены в calc.js:82-89, расчёт в calc.js:683.
Отправка заказа сейчас заглушка: calc.js:181-182.
Калькулятор подключён только на 2 страницах: foto-na-kholste-sankt-peterburg.html:1131-1133 и calc.html:219-229.
На страницах портретов в основном placeholder «Будет загружен из Bitrix» (пример: portret-maslom.html:179-186).
На live-страницах секция «Цена» с теми же типами опций действительно присутствует (по текстовому содержимому страниц).
Опровергаю / уточняю

Тезис «framemuse.ru точно редиректит на muse.ooo» сейчас не доказан инструментально; fetch вернул отдельный контент framemuse, без надёжной проверки HTTP-цепочки редиректа.
Цифры вида «1–5 мс», «100–500 мс», «Lighthouse 95–100» — это прогнозы, не замеры в репозитории. Их нужно пометить как оценку, не факт.
Важное дополнение

В Project есть логическое противоречие: одновременно «Рекомендация: Вариант 2 Hybrid» в PROJECT.md:655 и «Принято: Вариант 1» в PROJECT.md:756. Лучше оставить один статус как текущий decision, второй как future option.
Итоговый вывод

Архитектурно ваши выводы в целом верные (текущее состояние = V1 file-based, rollout через единый runtime).
Самое критичное для точности отчёта: убрать/смягчить недоказанные speed-цифры и явно развести «текущее решение» vs «целевая эволюция».
Хочешь, я отдельно подготовлю короткую формулировку (3–5 строк) для Project, чтобы снять противоречие V1/V2 без переписывания всего раздела?