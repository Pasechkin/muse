...
## Подготовка к передаче (ДО интеграции)

Перед передачей проекта программисту выполните следующие шаги:
...
Это создаст скомпилированный `src/html/css/output.css` со всеми стилями.

### 2. Замена CDN на локальный CSS

В каждой HTML странице заменить:
...
- [ ] Открыть каждую страницу в браузере
- [ ] Проверить отображение на мобильных (DevTools → 375px)
- [ ] Проверить работу каруселей
- [ ] Проверить мобильное меню
- [ ] Проверить кнопку "Наверх"

### 4. Что передать программисту

| Файл/Папка | Описание |
|------------|----------|
| `src/html/` | Все готовые HTML страницы |
| `src/html/css/output.css` | Скомпилированный CSS |
| `src/html/js/scripts.js` | Общие скрипты |
| `src/design-system/` | CSS переменные (для справки) |
| `docs/` | Документация |
| Этот файл | Инструкция по интеграции |

### 5. Убрать защиту от индексации

После полного переноса на Битрикс:

1. Удалить `robots.txt` или заменить на разрешающий
2. Убрать `<meta name="robots" content="noindex, nofollow">` из страниц
