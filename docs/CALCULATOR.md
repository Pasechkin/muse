...
9. [Архитектура: варианты и приоритеты](#9-архитектура-варианты-и-приоритеты)
10. [Требования к серверной части](#10-требования-к-серверной-части)
11. [Удобство управления ценами](#11-удобство-управления-ценами)

---

## 1. Обзор системы

Калькулятор — клиентский JS-компонент для расчёта стоимости печати на холсте и портретов на заказ. Пользователь выбирает параметры (размер, подрамник, покрытия, багет), загружает фото и получает итоговую цену.

### Из чего состоит

```
prices.js          ← Единый конфиг цен + тексты подсказок
    ↓
calc.js            ← Движок калькулятора (UI, логика, формулы)
    ↑
...
3. На странице вызывается `CalcInit({ type, prices, tooltips })` → создаётся UI
4. `CalcInit` внутри создаёт экземпляр `MuseUploader()` для загрузки фото
...
| Параметр | `type: 'canvas'` | `type: 'portrait'` |
|----------|-------------------|---------------------|
| Страницы | Фото на холсте, Печать на холсте | Все стили и объекты портретов |
| Обработка фото | 3 уровня (Базовая / Оптимальная / Премиальная) | Не используется |
| Количество лиц | — | Select 1–10 |
| Покрытия | Лак, Упаковка | Лак, Гель, Акрил, Масло, Поталь, Упаковка |
| Цифровой макет | — | Чекбокс (отключает физические опции) |
| Формула | Площадь × коэфф. + периметр × коэфф. + опции | То же + стоимость лиц + покрытия |
...
|------|--------|------------|
| `src/html/js/prices.js` | ~155 строк | Единый конфиг цен (`MUSE_PRICES`) + тексты подсказок (`MUSE_TOOLTIPS`) |
| `src/html/js/calc.js` | ~1 744 строки | Движок калькулятора: UI, формулы, каталог рам, визуализатор |
...
<!-- Инициализация (после всех скриптов) -->
<script defer>
  document.addEventListener('DOMContentLoaded', function () {
    CalcInit({
      type: 'canvas',                      // или 'portrait'
      prices: MUSE_PRICES.canvas,           // или MUSE_PRICES.portrait
      tooltips: MUSE_TOOLTIPS.canvas        // или MUSE_TOOLTIPS.portrait
    });
  });
</script>
```

> **Важно:** `prices.js` должен загрузиться до `calc.js`, иначе `MUSE_PRICES` будет undefined и калькулятор будет использовать нулевой fallback (`MINIMAL_FALLBACK`).
...
Единая точка входа — глобальная функция `CalcInit(cfg)`.

### Параметры конфигурации

| Параметр | Тип | Обязателен | По умолчанию | Описание |
|----------|-----|------------|--------------|----------|
| `type` | `string` | Нет | `'canvas'` | Тип калькулятора: `'canvas'` (печать на холсте) или `'portrait'` (портрет) |
| `prices` | `object` | **Да** | `MINIMAL_FALLBACK` (всё по нулям) | Объект цен из `MUSE_PRICES.canvas` или `MUSE_PRICES.portrait` |
| `tooltips` | `object` | Нет | `null` | Тексты подсказок из `MUSE_TOOLTIPS.canvas` или `MUSE_TOOLTIPS.portrait` |
...
**`shared`** — общие параметры, которые используются и в canvas, и в portrait:
...
- Багет (цена за погонный метр, множитель для классических)
...
Все формулы — в функции `calculate()` файла `calc.js` (строка ~1436).
...
Подробный контракт API → [раздел 10.3](#103-каталог-рамок).
...
**→ Интеграционное требование:** серверный endpoint для приёма файлов → см. [раздел 10](#10-требования-к-серверной-части).
...
**→ Интеграционное требование:** заменить на `fetch()` к серверному endpoint → см. [раздел 10](#10-требования-к-серверной-части).

---

## 9. Архитектура: варианты и приоритеты
...
1. Серверный endpoint `GET /api/frames.json` → контракт в [разделе 10.3](#103-каталог-рамок)
...
- `POST /api/order` — приём заказа → [раздел 10.1](#101-отправка-заказа)
- `POST /api/upload` — загрузка фото → [раздел 10.2](#102-загрузка-фото)
...
#### 10.4. Конфиг цен (на будущее, при переходе к Hybrid-full)
...
**Требования:**
- Endpoint `GET /api/prices.json` — формат совпадает с `MUSE_PRICES` (см. [§10.4](#104-цены-api))
...
| Термин | Значение |
|--------|----------|
| **calc.html** | Эталонная демо-страница калькулятора (source of truth для разметки) |
| **CalcInit** | Глобальная JS-функция — единая точка входа для инициализации калькулятора |
| **MuseUploader** | Глобальная JS-функция — загрузчик изображений |
| **MUSE_PRICES** | Глобальный объект с ценами (из `prices.js`) |
| **MUSE_TOOLTIPS** | Глобальный объект с текстами подсказок (из `prices.js`) |
| **MINIMAL_FALLBACK** | Нулевой конфиг цен в `calc.js` — используется, если `prices.js` не загружен |
| **DEFAULT_FRAMES** | Массив 27 рамок в `calc.js` — каталог багетов |
| **STATE** | Внутренний объект состояния калькулятора (размер, опции, выбранная рамка и т.д.) |
