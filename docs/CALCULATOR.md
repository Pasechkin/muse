...
> **Дата:** 24 февраля 2026 (обновлено: июль 2025 — рефакторинг helpers, frames.js, удаление alert/console.log)
...
11. [Калькулятор репродукций (`reproduction-search.js`)](#11-калькулятор-репродукций-reproduction-searchjs)
12. [Lazy-инициализация](#12-lazy-инициализация)
13. [Глобальные переменные и хуки](#13-глобальные-переменные-и-хуки)
14. [Реестр страниц с калькулятором](#14-реестр-страниц-с-калькулятором)
15. [Архитектура: варианты и приоритеты](#15-архитектура-варианты-и-приоритеты)
16. [Требования к серверной части](#16-требования-к-серверной-части)
17. [Удобство управления ценами](#17-удобство-управления-ценами)

---

## 1. Обзор системы

Система калькуляторов — набор клиентских JS-компонентов для расчёта стоимости продуктов Muse. Включает **4 типа** единого калькулятора (`CalcInit`), **отдельный модульный конструктор** (`modular-calc.js`), **конструктор фотоколлажей** (iframe-интеграция) и **калькулятор репродукций** (модальный, встроен в `reproduction-search.js`). Пользователь выбирает параметры (размер, подрамник, покрытия, багет), загружает фото и получает итоговую цену.
...
reproduction-search.js ← Поиск репродукций (5 API) + модальный калькулятор (IIFE)
    ↑
prices.js              ← Использует MUSE_PRICES.canvas (общие коэффициенты)
    ↑
frames.js              ← Каталог рам (MUSE_FRAMES)
```
...
3. `IntersectionObserver` отслеживает видимость секции `#calc` → при приближении к viewport вызывает `CalcInit()` (lazy-инициализация, см. [§12](#12-lazy-инициализация))
...
6. Пользователь заполняет форму → отправка (сейчас `<dialog>` подтверждение; при интеграции с Bitrix — `fetch()` к API)
...
4. Заполняет форму `#mc-order-form` → отправка (`<dialog>` подтверждение; заглушка до интеграции с Bitrix)

### Семь вариантов калькулятора
...
| **Репродукция** | — (IIFE) | `reproduction-search.js` | `reproduktsiya` | Как canvas (S×коэфф + периметр) + рамка + лак + упаковка | Поиск по 5 музеям, longSide-размер, aspect ratio, mailto |

### Сравнение опций по типам

| Опция | `canvas` | `portrait` | `frame` | `portraitStyle` | Модульный | Репродукция |
|-------|----------|------------|---------|-----------------|-----------|------------|
| Размер (ВхШ) | ✓ | ✓ | ✓ (мин. 20×30) | ✓ | ✓ (формат+раскладка) | longSide 40/60/80/100 + слайдер 30–120 |
| Подрамник (3 типа) | ✓ | ✓ | — | ✓ | Только галерейный | ✓ (3 типа) |
| Обработка фото | 3 уровня | — | 3 уровня | — | 3 уровня | — |
| Количество лиц | — | Select 1–10 | — | Select 1–10 | — | — |
| Выбор стиля | — | — | — | Select 21 стиль | — | — |
| Паспарту | — | — | ✓ (чекбокс) | — | — | — |
| Лак | ✓ | ✓ | — | ✓ | ✓ (по суммарной S) | ✓ (вкл. по умолч.) |
| Гель / Акрил / Масло / Поталь | — | ✓ | — | — | — | — |
| Цифровой макет | — | ✓ | — | — | — | — |
| Багет | ✓ | ✓ | ✓ (по умолч.) | ✓ | — | ✓ (STUDIO + CLASSIC) |
| Подарочная упаковка | ✓ | ✓ | ✓ | ✓ | ✓ (× кол-во модулей) | ✓ |
| Загрузка фото | ✓ | ✓ | ✓ | ✓ | ✓ | — (фото из музея) |
...
| `src/html/js/calc.js` | ~2 560 строк | Движок калькулятора: UI, формулы для 4 типов, каталог рам, визуализатор |
| `src/html/js/frames.js` | ~56 строк | Каталог демо-рамок (`DEFAULT_FRAMES`, 27 шт.) + константы `PRICE_STUDIO` / `PRICE_CLASSIC` |
...
| `src/html/js/reproduction-search.js` | ~1 445 строк | Поиск репродукций (5 музейных API) + модальный калькулятор: IIFE, автоинит |
...
**Для репродукций** (другой порядок, без uploader/calc):

```html
<script defer src="../js/nav.js"></script>
<script defer src="../js/prices.js"></script>
<script defer src="../js/frames.js"></script>
<script defer src="../js/reproduction-search.js"></script>
```

> `reproduction-search.js` заменяет `calc.js` и `uploader.js` — содержит собственный калькулятор в `<dialog>` модалке. Загрузка фото не нужна: пользователь выбирает картину из музейного каталога.

> `modular-calc.js` заменяет `calc.js` — используется **вместо** него, а не вместе.

### Версионирование скриптов

Все страницы единообразно подключают `calc.js?v=3`.
...
### Внутренние хелперы `calc.js` (после рефакторинга)

В ходе оптимизации крупные повторяющиеся блоки были вынесены в переиспользуемые функции:

#### UI-хелперы (строят секции калькулятора)

| Функция | Строка | Назначение |
|---------|--------|------------|
| `_createChevronSvg()` | ~375 | SVG-иконка стрелки для `<select>` |
| `_detach(el)` | ~393 | Безопасное удаление элемента из DOM (возвращает элемент) |
| `makeInlineCheckboxSection(id, label, hintKey, badgeId, checked, opts)` | ~399 | Универсальный inline-чекбокс (лак, упаковка, паспарту) |
| `_portraitCommonSetup()` | ~467 | Общая инициализация для `portrait` и `portraitStyle` (обработка покрытий, лица, совместимость) |
| `_buildDropdownSection(selectId, label, hintKey, badgeId, badgeText, options)` | ~540 | Универсальный `<select>` с бейджем и подсказкой |

#### Build-функции (собирают UI секций)

| Функция | Строка | Тип |
|---------|--------|-----|
| `buildPortraitSections()` | ~582 | `portrait` — покрытия, лица, цифровой макет |
| `buildPortraitStyleSections()` | ~734 | `portraitStyle` — стиль + лица |
| `buildFrameSections()` | ~844 | `frame` — паспарту + дефолтная рамка |

#### Расчётные хелперы

| Функция | Строка | Назначение |
|---------|--------|------------|
| `calcGiftWrap(w, h)` | ~2071 | Расчёт стоимости подарочной упаковки по `giftWrapTiers` |
| `calcStretcherPrintCost(sq, perim)` | ~2090 | Базовая стоимость печати + подрамник (S×коэфф + P×коэфф + конст.) |

> **Примечание:** `buildPortraitSections` и `buildPortraitStyleSections` делят общую логику через `_portraitCommonSetup()`, а checkbox-секции (лак, упаковка, паспарту) — через `makeInlineCheckboxSection()`. Это устраняет дублирование ~200 строк между типами.
...
Подробный контракт API → [раздел 16.3](#163-каталог-рамок).

### Текущая демо-структура (для разработки)

27 рамок вынесены в отдельный файл `frames.js` (`DEFAULT_FRAMES`, экспортируется как `window.DEFAULT_FRAMES`). Константы цен: `PRICE_STUDIO = 1200`, `PRICE_CLASSIC = 1800`. Делятся на 2 категории:
...
**→ Интеграционное требование:** серверный endpoint для приёма файлов → см. [раздел 16](#16-требования-к-серверной-части).
...
После нажатия «Заказать» показывается `<dialog>` с текстом «Заказ оформлен. Спасибо!». Диалог использует тот же CSS-класс `calc-hint-dialog`, что и подсказки.
...
**→ Интеграционное требование:** добавить `fetch()` к серверному endpoint **перед** показом диалога → см. [раздел 16](#16-требования-к-серверной-части).
...
## 11. Калькулятор репродукций (`reproduction-search.js`)

### Обзор

Модальный калькулятор стоимости репродукций — часть файла `reproduction-search.js` (~1 445 строк, IIFE, ES5). Файл состоит из двух логических частей:

1. **Поисковый движок** (~строки 1–750) — мультимузейный поиск по 5 API. Документация: [PROJECT.md § Мультимузейный поиск репродукций](../PROJECT.md#мультимузейный-поиск-репродукций).
2. **Модальный калькулятор** (~строки 750–1445) — расчёт стоимости печати выбранной картины. Документация — этот раздел.

**Страница:** `src/html/pechat/reproduktsiya.html`

**Ключевое отличие от остальных калькуляторов:** пользователь не загружает своё фото, а выбирает картину из музейного каталога. Калькулятор открывается в `<dialog>` поверх результатов поиска. Заказ отправляется через `mailto:hello@muse.ooo`.

### Архитектура

```
reproduction-search.js (IIFE)
├── TRANSLATE_MAP (42 ru→en)
├── POPULAR_TAGS, LONG_SIDES, STRETCHER_TYPES
├── Ценовые функции: P(), calcCanvasPrice, calcVarnishPrice, calcGiftPrice, calcFramePrice
├── getDimensions(longSide) → {w, h} по aspect ratio выбранной картины
├── calcTotal() → {canvas, frame, varnish, gift, total, w, h, stretcher}
├── Поисковые провайдеры: metSearch, aicSearch, rijksSearch, cleveSearch, wikiSearch
├── UI поиска: masonry grid, retry, dedup, interleave
└── Модальный калькулятор:
    ├── openModal(painting) → <dialog>.showModal()
    ├── renderCalculatorOptions() → размеры, подрамник, рамы, лак, упаковка, итого
    ├── handleOrder() → mailto
    └── init() → привязка событий
    ↑
prices.js    → MUSE_PRICES.canvas (коэффициенты — тот же набор, что у CalcInit canvas)
    ↑
frames.js    → MUSE_FRAMES.DEFAULT_FRAMES, MUSE_FRAMES.getFramePrice()
```

### Жизненный цикл

1. Страница загружается → `prices.js` создаёт `MUSE_PRICES`, `frames.js` создаёт `MUSE_FRAMES`
2. `reproduction-search.js` загружается → `DOMContentLoaded` вызывает `init()`
3. `init()` инициализирует каталог рам (`initFramesCatalog()`), рендерит популярные теги, привязывает обработчики
4. Пользователь вводит запрос → параллельный поиск по 5 API → masonry-грид с результатами
5. Клик по карточке → `openModal(painting)` → `<dialog id="repro-modal">` открывается
6. Калькулятор определяет `aspectRatio` из загруженного изображения, рендерит опции
7. Пользователь выбирает размер, подрамник, раму, лак, упаковку → `renderCalculatorOptions()` пересчитывает цену
8. Нажатие «Заказать» → `handleOrder()` → валидация → `mailto:` с деталями заказа → модал закрывается через 2.5 сек

### Зависимости

| Файл | Что используется |
|------|-----------------|
| `prices.js` | `MUSE_PRICES.canvas.*` — все коэффициенты печати, подрамников, лака, упаковки |
| `frames.js` | `MUSE_FRAMES.DEFAULT_FRAMES` — каталог рам; `MUSE_FRAMES.getFramePrice()` — формула цены; `MUSE_FRAMES.getFramePreviewCSS()` — стиль превью |

> **Не использует:** `calc.js`, `uploader.js`, `CalcInit()`. Полностью автономный калькулятор.

### Выбор размера: longSide + aspect ratio

В отличие от остальных калькуляторов (где пользователь вводит ВхШ), здесь размер задаётся через **длинную сторону** — вторая сторона вычисляется автоматически по пропорциям оригинала.

**Пресеты размеров:**

| id | Длинная сторона | По умолчанию |
|----|----------------|-------------|
| `s` | 40 см | — |
| `m` | 60 см | ✓ |
| `l` | 80 см | — |
...
**Слайдер:** дополнительно — `<input type="range">` от 30 до 120 см с шагом 1.

**Логика `getDimensions(longSide)`:**
```javascript
if (aspectRatio >= 1) { w = longSide; h = round(longSide / aspectRatio); }
else                  { h = longSide; w = round(longSide * aspectRatio); }
```

`currentAspectRatio` определяется при загрузке изображения в модале (`naturalWidth / naturalHeight`). Если пропорции неизвестны — `w = h = longSide` (квадрат).

### Подрамники

| id | Название | Ключ в prices.js | Fallback |
|----|----------|-------------------|----------|
| `STANDARD` | Стандартный | `stretcherStandard` | 32 |
| `GALLERY` | Толстый | `stretcherGallery` | 68 |
| `NO_FRAME` | Рулон | `stretcherRoll` | 32 |

### Формулы расчёта

Формулы **идентичны** `calc.js` canvas-типу:

| Позиция | Формула | Пример 45×60 |
|---------|---------|--------------|
| **Печать + подрамник** | `printSqCoeff·S + printPStrCoeff·P·stretcherPrice + printPBaseCoeff·P + printConst` | ~3 197 р. (стандартный) |
| **Лак** | `S × varnishCoeff` (вкл. по умолчанию) | 270 р. |
| **Подарочная упаковка** | По `giftWrapTiers` (выкл. по умолчанию) | 750 р. (если вкл.) |
| **Багет** | `MUSE_FRAMES.getFramePrice(frame, w, h)` | зависит от рамы |
| **Итого** | `canvas + frame + varnish + gift` | ~3 467 р. (без рамы, с лаком) |

> **Значения по умолчанию при открытии модала:** longSide=60, подрамник=Стандартный, рама=NONE (без багета), лак=ВКЛ, упаковка=ВЫКЛ.

### Каталог рам

Использует тот же каталог из `frames.js`, что и `calc.js`:
- Рамы фильтруются по `available !== false`
- Разделены на STUDIO и CLASSIC
- Превью: CSS-стиль через `MUSE_FRAMES.getFramePreviewCSS()`
- Цена: через `MUSE_FRAMES.getFramePrice(frame, w, h)`
- При выборе рамы обновляется превью изображения (CSS-бордер) и пересчитывается цена

### Форма заказа (mailto)

В отличие от `calc.js` (где заказ — `<dialog>` подтверждение), здесь заказ отправляется через `mailto:`.

**Поля формы:**

| Поле | id | Обязательное | Валидация |
|------|----|-------------|-----------|
| Имя | `repro-name` | Да | Не пустое |
| Телефон | `repro-phone` | Да | Не пустое |
| Email | `repro-email` | Нет | — |
| Комментарий | `repro-comment` | Нет | — |

**Данные в письме:**
```
Картина: [название]
Художник: [автор]
Источник: [Met Museum / Art Institute / ...]
Размер: 45×60 см
Подрамник: Стандартный
Багет: Без багета
Покрытие: Лак включён
...
Ссылка на изображение: [URL из музея]
```

**После отправки:** кнопка меняется на «Заказ оформлен ✓» (зелёный фон), через 2.5 сек модал закрывается, форма очищается.

### DOM-контракт (обязательные id)

| id | Назначение |
|----|------------|
| `repro-modal` | `<dialog>` модальное окно калькулятора |
| `modal-title` | Название картины |
| `modal-artist` | Имя художника |
| `modal-original-size` | Размеры оригинала (Wikidata SPARQL) |
| `modal-image` | Изображение картины |
| `modal-frame` | Контейнер с CSS-рамкой (превью багета) |
| `sizes-container` | Контейнер пресетов размеров |
| `repro-size-slider` | `<input type="range">` слайдер размера |
| `slider-value` | Отображение текущего значения слайдера |
| `stretcher-container` | Контейнер кнопок подрамника |
| `frames-studio-container` | Контейнер рам STUDIO |
| `frames-classic-container` | Контейнер рам CLASSIC |
| `repro-varnish-toggle` | Чекбокс лака |
| `repro-gift-toggle` | Чекбокс подарочной упаковки |
| `modal-total-price` | Итоговая цена |
| `sticky-repro-price` | Цена в мобильном sticky-баре |
| `repro-order-btn` | Кнопка «Заказать» |
| `repro-order-text` | Текст кнопки заказа |
| `repro-name` | Поле имени |
| `repro-phone` | Поле телефона |
| `repro-email` | Поле email |
| `repro-comment` | Поле комментария |
| `close-modal-btn` | Кнопка закрытия модала |

### Поисковые элементы (вне модала)

| id | Назначение |
|----|------------|
| `search-form` | Форма поиска |
| `search-input` | Поле ввода запроса |
| `popular-tags` | Контейнер популярных тегов |
| `results-grid` | Masonry-грид результатов |
| `load-more-btn` | Кнопка «Показать ещё» |

### Инициализация

`reproduction-search.js` не использует `IntersectionObserver` — вызывает `init()` сразу при `DOMContentLoaded`:
...
Файл — IIFE, **не загрязняет глобальную область**. Всё состояние замкнуто в closure. Единственные внешние зависимости:
- `window.MUSE_PRICES` (из `prices.js`)
- `window.MUSE_FRAMES` (из `frames.js`)

---

## 12. Lazy-инициализация
...
## 13. Глобальные переменные и хуки
...
## 14. Реестр страниц с калькулятором
...
| `src/html/pechat/reproduktsiya.html` | — (IIFE) | prices, frames, **reproduction-search** | Поиск по 5 музеям + модальный калькулятор | — (сразу) |
...
| `portret-na-zakaz-po-foto-*` | `nav → uploader → prices → frames → calc.js?v=3` |
| `modulnaya-kartina` | `nav → prices → uploader → modular-calc.js` |
| `reproduktsiya` | `nav → prices → frames → reproduction-search.js` |

---

## 15. Архитектура: варианты и приоритеты
...
1. Серверный endpoint `GET /api/frames.json` → контракт в [разделе 16.3](#163-каталог-рамок)
...
- `POST /api/order` — приём заказа → [раздел 16.1](#161-отправка-заказа)
- `POST /api/upload` — загрузка фото → [раздел 16.2](#162-загрузка-фото)
...
## 16. Требования к серверной части
...
#### 16.4. Конфиг цен (на будущее, при переходе к Hybrid-full)
...
**Требования:**
- Endpoint `GET /api/prices.json` — формат совпадает с `MUSE_PRICES` (см. [§16.4](#164-конфиг-цен-на-будущее-при-переходе-к-hybrid-full))
