...
- при наличии компонента добавить его критические классы: `.page-navigator`, `.ba-card`, `.carousel-scroll`.
...
**Исключение:** если на первом экране видео или нет видимого изображения (LCP = текст/видео/иконки), правило для LCP-изображения **не применяется**. Не ставим `fetchpriority="high"` на изображения ниже первого экрана.
...
Текстовые ссылки **внутри абзацев** обязательно с подчёркиванием для различимости. Навигация, кнопки и карточки используют свои стили из дизайн-системы.
...
### 3D Эффект (нативные утилиты Tailwind)

Используем нативные утилиты Tailwind для легкого 3D‑эффекта без кастомного CSS.

**HTML:**
```html
<div class="perspective-distant">
    <img src="img/portrait.jpg" alt="Портрет на холсте"
         class="w-full transform-3d rotate-x-[1deg] rotate-y-[-1deg] shadow-[24px_24px_40px_rgba(0,0,0,0.35)]
                transition-transform duration-300 ease-out hover:-translate-x-[4px] hover:-translate-y-[4px]
                hover:rotate-x-[0deg] hover:rotate-y-[0deg]">
</div>
```
...
### 3D Эффект (Tailwind utils)

3D эффект для изображений через `perspective-*`, `transform-3d`, `rotate-*` и кастомную тень.
...
- **Исключение:** если на первом экране видео или нет видимого изображения (LCP = текст/видео/иконки), правило для LCP-изображения **не применяется**. Считаем LCP по фактическому первому экрану (desktop/mobile), не назначаем `fetchpriority="high"` ниже первого экрана.
...
Используем для секций **ниже первого экрана**, чтобы ускорить рендер больших страниц.

**Правила:**
- Добавляем класс `content-auto` на секции ниже первого экрана.
- **Не применять** к hero, первой секции после hero (может быть видна на больших экранах), promo-баннерам и CTA в конце страницы.
- Если секция на границе первого экрана (desktop/mobile), класс не ставим.
...
### Визуальные эффекты
- [ ] 3D эффект: Tailwind utils (`perspective-*`, `transform-3d`, `rotate-*`, `shadow-[...]`)
