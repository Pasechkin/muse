...
| Footer | `text-white/70 hover:text-white` | Нижний колонтитул |
...
Все модальные окна на светлом фоне используют **один и тот же** крестик закрытия:

```html
<button type="button" class="p-2 rounded-lg text-ink-muted hover:text-ink transition-colors"
        data-close-dialog aria-label="Закрыть">
...
|----------|----------|------------|
| Padding | `p-2` | Touch-target 40×40px при иконке 24×24 |
| Цвет | `text-ink-muted` → `hover:text-ink` | ah-700 → ah-950 (ink-система) |
| Скругление | `rounded-lg` | Мягкий квадрат |
| SVG stroke | `1.5`, `round` linecap/join | Тонкий, элегантный штрих |
| Доступность | `aria-label="Закрыть"`, `aria-hidden="true"` на SVG | |

**Применяется к:** City dialog, Callback dialog, Frame (baguette) modal, Lightbox, Review modal.
